...
const MessageStatusIndicator: React.FC<{ status: MessageStatus, onClick?: () => void }> = ({ status, onClick }) => {
    switch (status) {
        ...
        case MessageStatus.Failed:
            return <span onClick={() => onClick && onClick()}>Failed. Click To Resend.</span>;
    }
}
...
const UserChatBubble: React.FC<UserChatBubbleProps> = ({ messageString, messageStatus, onSystemMessage }) => {
    ...
    return (
        <React.Fragment>
            ...
            <MessageStatusIndicator status={currentMessageStatus} onClick={handleMessageReasend} />
        </React.Fragment>
    );
}

const SystemChatBubble: React.FC<ChatBubbleMessageProps> = ({ messageString }) => {
    return (
...
const ChatBubble: React.FC<ChatBubbleProps> = ({
    ...
    return sender_type === 'USER' ? <UserChatBubble messageString={message} messageStatus={status} onSystemMessage={onSystemMessage} /> : <SystemChatBubble messageString={message} messageStatus={MessageStatus.Sent} />;
};
